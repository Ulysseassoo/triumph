...
export class OrderController {
  ...
  @Post()
  async create(@Body() orderData: { pieces:object[],orderDate:string; deliveryDate:string; status: string; totalAmount: number }): Promise<Order> {
  ...
  @Get(':date')
  async findByOrderDate(@Param('date') date: string): Promise<Order> {
    return await this.orderService.findByOrderDate(date);
  }
  @Get(':date')
  async findByDeliveryDate(@Param('date') date: string): Promise<Order> {
    return await this.orderService.findByDeliveryDate(date);
  }
  @Get(':totalAmount')
  async findByTotalAmount(@Param('totalAmount') totalAmount: number): Promise<Order> {
