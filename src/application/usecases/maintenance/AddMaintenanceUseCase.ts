...
import { Maintenance, MaintenanceType } from "./../../../domain/entities/maintenance.entity"
...
export class AddMaintenanceUseCase {
	...
	async execute({ motoId, kilometrageInterval, tempsInterval, recommandations }: Props): Promise<Maintenance | null> {
		...
		await this.sendNotificationUseCase.execute(moto.clientId, `Un entretien a été planifié pour votre moto (${moto.model})`)
